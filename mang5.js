...
function khuyenNghiHanhDong(cd, dt, dtHao, theHao, ungHao) {
    ...
    switch (cd) {
        case 'hon_nhan':
            ...
            break;
        case 'ban_hang':
            if (dtHao.isVuong && dtHao.lt === 'Tử Tôn') { kq = '📦 <b>Hàng còn ứ đọng.</b> Cần đẩy mạnh marketing, giảm giá, hoặc tìm kênh phân phối mới. Hàng Vượng = khó bán.'; }
            else if (!dtHao.isVuong || dtHao.tk === 'K') { kq = '💰 <b>Thời điểm tốt để bán!</b> Hàng hóa đang có cơ hội ra đi. Nên chốt deal nhanh, tránh do dự.'; }
            break;
        case 'cong_danh':
...
function renderMang5(dt, dtHao, theHao, ungHao, listNT, listKT, dongArrList) {
    ...
    h += `<select id="sel-chu-de" style="width:100%;padding:5px;border:1px solid #dcdfe4;border-radius:4px;font-size:clamp(9px,0.72vw,11px);background:#fff;cursor:pointer;margin:6px 0 4px">
        ...
        <option value="ban_hang">💰 Bán Hàng / Cầu Tài</option>
...
function phanTichChuDeChuyenSauKhac(cd, dt, dtHao, theHao, ungHao, listNT, listKT, dongArrList) {
    ...
    const chiNgay = dt.cNgay.split('-')[1];
    const timHao = (lt) => dt.haoData.find(h => h.lt === lt);

    switch (cd) {
        case 'ban_hang':
            res += `<div style="font-weight:700;color:var(--blue);margin-bottom:4px;font-size:12px">💰 LUẬN GIẢI BÁN HÀNG / CẦU TÀI</div>`;
            res += `<p style="font-size:10px;color:#888;margin-bottom:4px"><i>Bán hàng ≠ Cầu tài lớn. Tử Tôn = Hàng hóa (cần Suy/Không/Khắc để dễ đi). Thê Tài = Lợi nhuận (cần Vượng sinh Thế).</i></p>`;
            ...
            if (tuTonBH) { if (tuTonBH.i === theHao.i) res += `<li>Tử Tôn tại Thế → Hàng trong tay mình. ${tuTonBH.isVuong ? '<b>Vượng = KHÓ BÁN</b>' : '<b>Suy = DỄ BÁN</b>'}</li>`; else if (tuTonBH.i === ungHao.i) res += `<li>Tử Tôn tại Ứng → Hàng tới tay đối phương. ${tuTonBH.isVuong ? '<b>Vượng = CÓ LỢI</b>' : '<b>Suy = Bất lợi</b>'}</li>`; else res += `<li>Hàng hóa: ${tuTonBH.isVuong ? 'Vượng (khó bán)' : 'Suy (dễ bán)'}</li>`; } else res += `<li>Không có Tử Tôn → Mặt hàng không rõ ràng.</li>`;
            res += `</ul>`;
            res += `<div style="font-weight:600;font-size:11px;color:var(--purple);margin-top:4px">II. LỢI NHUẬN (Thê Tài):</div><ul style="padding-left:16px;margin-top:2px">`;
            if (theTaiBH) { if (theTaiBH.isVuong && (NH_SINH[theTaiBH.nh] === theNh || theTaiBH.i === theHao.i)) res += `<li><b>🟢 Vượng sinh Thế / trì Thế:</b> <b>RẤT MAY MẮN!</b> Bán giá cao, lợi nhuận lớn.</li>`; else if (theTaiBH.tk === 'K' && theTaiBH.i === theHao.i) res += `<li><b>🔴 Tài Không Phá tại Thế:</b> <b>NGUY HIỂM!</b> Không bán nợ, phải thu tiền trước!</li>`; else res += `<li>Hưu tù → Lợi nhuận <b>không lớn</b>, giá thấp.</li>`; } else res += `<li>Không có Thê Tài → Lợi nhuận không rõ.</li>`;
            res += `</ul>`;
            let phuMauBH = timHao('Phụ Mẫu');
            if (phuMauBH) { res += `<div style="font-weight:600;font-size:11px;color:var(--purple);margin-top:4px">III. BÁN NHÀ (nếu áp dụng):</div><ul style="padding-left:16px;margin-top:2px">`; res += `<li>Phụ Mẫu = Nhà. Bị Không/Phá/Khắc → 🟢 <b>Có thể bán ra.</b></li>`; if (theTaiBH && theTaiBH.isVuong && (theTaiBH.i === theHao.i || NH_SINH[theTaiBH.nh] === theNh)) res += `<li>Tài vượng sinh Thế → 🟢 <b>Bán giá cao.</b></li>`; res += `</ul>`; }
            let huynhDeBH = timHao('Huynh Đệ');
            if (huynhDeBH && huynhDeBH.dong && huynhDeBH.isVuong) res += `<p style="color:#d32f2f;margin-top:4px"><b>🔴 Huynh Đệ động + Vượng:</b> Cạnh tranh gay gắt, bị phá giá, mất khách!</p>`;
            break;
        case 'tim_nguoi':
