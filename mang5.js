...
function renderMang5(dt, dtHao, theHao, ungHao, listNT, listKT, dongArrList) {
    ...
    if (dt.qTinhChat.includes("Lục Xung")) { h += `<p class="m5-p color-bad" style="margin-top:4px"><b>⚡ Quẻ Lục Xung:</b> Mọi việc dễ đổ vỡ, chia ly, biến động đột ngột. <i>Hôn nhân: bất hòa. Sinh đẻ: dễ sinh. Kiện tụng: ly tán. Cầu tài: Đánh nhanh thắng nhanh.</i></p>`; }
    if (dt.qTinhChat.includes("Lục Hợp")) { h += `<p class="m5-p color-warn"><b>🔗 Quẻ Lục Hợp:</b> Sự việc dùng dằng, níu kéo, khó dứt điểm. <i>Hôn nhân: ổn định. Cầu tài: Hợp tác lâu dài tốt. Tìm người: bị níu chân.</i></p>`; }
    ...
    if (QUE_VONG.includes(dt.tenQue)) { h += `<p class="m5-p m5-highlight"><b>👻 Quẻ phần Âm (Nhà có vong):</b> Quẻ này thuộc 24 quẻ Âm, dễ có sự can thiệp của vong linh. Kiểm tra mồ mả, bàn thờ gia tiên.</p>`; }
...
function phanTichChuDeChuyenSauKhac(cd, dt, dtHao, theHao, ungHao, listNT, listKT, dongArrList) {
    ...
    switch (cd) {
        case 'cau_tai': 
            ...
            else if (quan && quan.i === theHao.i && tai && tai.dong && NH_SINH[tai.nh] === theNh) res += `<li><b>🟢 Quan trì Thế, Tài động sinh Thế:</b> Cầu tài dễ dàng (có thể dựa vào chức vụ, quản lý, cơ quan công quyền để ra tiền).</li>`;
            ...
            else if (theHao.lt === 'Huynh Đệ') res += `<li><b>🔴 Huynh Đệ trì Thế:</b> Tượng khắc Tài, phá tài, làm ăn khó khăn, dễ mất tiền.</li>`;
            
            if(huynh && huynh.dong) res += `<li><b>🔴 Huynh Đệ phát động:</b> Cạnh tranh khốc liệt, có kẻ cướp đoạt lợi nhuận, tốn kém nhiều chi phí ngoài luồng.</li>`;
            ...
            if (huynh && huynh.dong) res += `<span class="color-bad">Huynh động thì chi phí cao, cướp tài, cạnh tranh cùng ngành khốc liệt.</span>`;
            else if (tu && tu.isVuong) res += `<span class="color-ok">Tử Tôn vượng là nguồn khách hàng đông đảo, buôn may bán đắt.</span>`;
            res += `</li>`;

            if (quan && quan.dong && quan.ltt === 'Huyền Vũ' && NH_KHAC[quan.nh] === theNh) {
                res += `<li><b>⚠️ Kinh doanh tạm thời/Môi giới:</b> Quan Quỷ phát động lâm Huyền Vũ khắc Thế → Đề phòng bị người môi giới lừa đảo, mất tiền oan!</li>`;
            }

            if (theHao.lt === 'Huynh Đệ' && ungHao.lt === 'Huynh Đệ') res += `<li><b>💸 Vay mượn/Đòi nợ:</b> Thế Ứng đều là Huynh Đệ (đồng nhân) → Khoản nợ này chắc chắn khó đòi lại.</li>`;
            if (ungHao.tk === 'K') res += `<li><b>💸 Đòi nợ:</b> Ứng lâm Không Vong → Đối phương lẩn tránh, hứa suông không chịu trả, đòi nợ thất vọng.</li>`;
            ...
            if (theBiHinhKhac) res += `<li><b>🎲 Cờ bạc/Đầu cơ rủi ro:</b> Thế gặp Hình hoặc Khắc từ Nhật/Nguyệt/Động → Đánh bạc, đầu cơ chắc chắn sẽ thua.</li>`;

            res += `</ul><div style="font-weight:600;font-size:11px;color:var(--purple);margin-top:4px">III. BÍ QUYẾT LỤC HÀO CẦU TÀI:</div><ul style="padding-left:16px;margin-top:2px;list-style:square">`;
            
            if (tai && tai.isVuong && tu && tu.isVuong) res += `<li><b>🌟 1. Tài vượng Phúc hưng:</b> Dù làm công hay tư đều như ý, tiền của cuồn cuộn.</li>`;
            if (tai && tai.tk === 'K' && tu && tu.tsNgay === 'Tuyệt') res += `<li><b>🔴 2. Tài Không Phúc Tuyệt:</b> Kinh doanh đi ngược lại mong muốn, lỗ vốn thảm hại.</li>`;
            
            let isTaiPhucPhuc = (!tu || dt.phucThanData[tu?.i] !== undefined);
            if (tai && isTaiPhucPhuc && quan && quan.dong) res += `<li><b>🟡 3. Có Tài không Phúc, Quan phát động:</b> Vẫn có thể cầu (vì Quan động sẽ khắc chế Kỵ thần Huynh đệ, bảo vệ được hào Tài).</li>`;
            
            let isTaiPhucTai = (!tai || dt.phucThanData[tai?.i] !== undefined);
            if (tu && isTaiPhucTai && huynh && huynh.dong) res += `<li><b>🟡 4. Có Phúc không Tài, Huynh động:</b> Cầu tài có hy vọng (tuy chậm chạp) vì Huynh động sinh Tử Tôn, không khắc được hào Tài đang tàng phục.</li>`;
            
            if (isTaiPhucTai && isTaiPhucPhuc) res += `<li><b>🔴 5. Tài Phúc đều không xuất hiện:</b> Cầu tài vô ích, chẳng khác gì "ôm cây đợi thỏ".</li>`;
            
            if (phu && phu.dong && huynh && huynh.dong) res += `<li><b>🔴 6. Phụ và Huynh cùng động:</b> Chẳng khác gì "leo cây tìm cá", tuyệt đối không thể cầu tài (Phụ khắc Tử, Huynh khắc Tài).</li>`;
            
            if(tai && NH_KHAC[chiNgay] === tai.nh && tai.isVuong) {
                res += `<li><b>⏳ 7. Nhật thương Thê vị:</b> Ngày khắc hào Tài, tuy Tài vượng ở tháng nhưng không thể có ngay trong ngày, phải chờ ngày khác.</li>`;
            }

            let countTai = dt.haoData.filter(h=>h.lt==='Thê Tài').length;
            if(countTai >= 2 && tai && tai.isVuong) {
                res += `<li><b>💰 8. Tài nhiều cần Mộ:</b> Trong quẻ hào Thê Tài nhiều lại vượng, cần chờ gặp Mộ khố ở ngày/tháng mới có thể thu gom đắc tài.</li>`;
            }

            if(!quan) res += `<li><b>⚠️ 9. Không Quỷ tranh giành:</b> Quẻ không có Quan Quỷ, Kỵ thần (Huynh đệ) không bị chế ngự. Nếu Kỵ thần gặp nhiều sẽ vây hãm cướp tài.</li>`;
            if(quan && quan.dong) res += `<li><b>⚠️ 10. Quan Quỷ phát động:</b> Gây trở ngại và tổn hao, thị phi. Quẻ làm ăn kỵ Quan phát động.</li>`;

            if(tu && tu.dong && dt.bqData && dt.bqData[tu.i].lt === 'Thê Tài' && (NH_SINH[tai.nh] === theNh || HOP_MAP[tai.chi] === theChi)) {
                res += `<li><b>🌟 11. Phúc biến Tài sinh:</b> Tử tôn hóa Tài lại sinh hợp Thế → Lợi nhuận lâu dài, tiền tài liên tục đổ về không bao giờ cạn.</li>`;
            }

            if(tai && (NH_SINH[tai.nh] === theNh || HOP_MAP[tai.chi] === theChi || tai.i === theHao.i)) {
                res += `<li><b>🟢 12. Tài tới với Ta:</b> Tài sinh/hợp/trì Thế → Tiền tài tự tìm đến, chuyện cầu tài dễ dàng trong tầm tay.</li>`;
            } else if (tai && NH_KHAC[theNh] === tai.nh) {
                res += `<li><b>🟡 13. Ta đi tìm Tài:</b> Thế khắc Tài → Tất là chuyện khó, phải chủ động bôn ba vất vả mới kiếm được lợi nhuận.</li>`;
            }

            if(phu && phu.dong && dt.bqData && (dt.bqData[phu.i].lt === 'Tử Tôn' || dt.bqData[phu.i].lt === 'Thê Tài')) {
                res += `<li><b>🟡 14. Phụ hóa Tử/Tài:</b> Hào Phụ động hóa ra Tử Tôn hoặc Thê Tài → Làm việc vất vả lao lực mới có được tiền.</li>`;
            }

            if(theHao.tk === 'K' && ungHao.tk === 'K' && (HOP_MAP[theChi] === ungChi || dt.bqData?.[theHao.i]?.tk === 'K')) {
                res += `<li><b>🔴 15. Thế Ứng Không Hợp:</b> Là hoang đường, giả dối, bánh vẽ. Không có căn cứ thực tế, dễ bị lừa.</li>`;
            }

            if(tai && tai.dong && HOP_MAP[chiNgay] === tai.chi) {
                res += `<li><b>🔒 16. Nhật hợp động Tài:</b> Hào Tài động bị Ngày hợp → Tiền này đang bị người khác/cơ quan khác nắm giữ không buông, phải chờ đợi.</li>`;
            }

            res += `</ul><div style="font-weight:600;font-size:11px;color:var(--purple);margin-top:4px">IV. BẢN CHẤT DÒNG TIỀN & PHƯƠNG HƯỚNG:</div><ul style="padding-left:16px;margin-top:2px;list-style:square">`;

            // Lục thú áp dụng cho Cầu Tài
            if (tai) {
                let lttT = tai.ltt;
                if (lttT === 'Thanh Long') res += `<li><b>🐉 Tài lâm Thanh Long:</b> Tiền kiếm được một cách chính đáng, hợp pháp. Dễ thu lời từ đồ trang trí, hàng cao cấp, mỹ phẩm, ăn uống hoặc khách hàng sang trọng.</li>`;
                if (lttT === 'Chu Tước') res += `<li><b>🐦 Tài lâm Chu Tước:</b> Dễ kiếm tiền qua tài ăn nói (môi giới, tư vấn, giáo dục, pháp lý). HOẶC cẩn thận dễ sinh cãi vã, kiện tụng vì tiền bạc.</li>`;
                if (lttT === 'Câu Trần') res += `<li><b>🟤 Tài lâm Câu Trần:</b> Nguồn tiền đến từ đất đai, bất động sản, nông nghiệp, công trình xây dựng. Thu tiền thường bị chậm chạp, dây dưa, ngâm vốn.</li>`;
                if (lttT === 'Đằng Xà') res += `<li><b>🐍 Tài lâm Đằng Xà:</b> Dòng tiền thất thường, lúc có lúc không. Cẩn thận dùng thủ đoạn, hoặc đối tác tráo trở, lừa lọc ngầm.</li>`;
                if (lttT === 'Bạch Hổ') res += `<li><b>🐯 Tài lâm Bạch Hổ:</b> Lợi nhuận đi kèm rủi ro (đầu cơ, y tế, nghề nguy hiểm, đòi nợ). Kiếm tiền nhanh nhưng dễ hỏng việc vì nóng vội hoặc bị tai nạn hao tài.</li>`;
                if (lttT === 'Huyền Vũ') res += `<li><b>🐢 Tài lâm Huyền Vũ:</b> Tiền từ kênh mờ ám, không chính thức, trốn thuế, hoặc làm mảng giải trí, thủy sản. Đề phòng hao tài vì tửu sắc hoặc bị trộm cắp.</li>`;
                
                let phuongVi = { 'Tý': 'Bắc', 'Sửu': 'Đông Bắc', 'Dần': 'Đông Bắc', 'Mão': 'Đông', 'Thìn': 'Đông Nam', 'Tỵ': 'Đông Nam', 'Ngọ': 'Nam', 'Mùi': 'Tây Nam', 'Thân': 'Tây Nam', 'Dậu': 'Tây', 'Tuất': 'Tây Bắc', 'Hợi': 'Tây Bắc' };
                res += `<li><b>📍 Hướng Đầu Tư / Khách Hàng:</b> Hướng đắc lợi nhất là <b>${phuongVi[tai.chi]}</b> hoặc hợp tác với người tuổi <b>${tai.chi}</b>.</li>`;
            } else if (!tai) {
                res += `<li><b>📍 Nguồn Tiền Bị Ẩn (Tài Phục Tàng):</b> `;
                let phucHao = dt.haoData.find(h => dt.phucThanData[h.i] && dt.phucThanData[h.i].lt === 'Thê Tài');
                if (phucHao) {
                    let phiHao = phucHao; 
                    if (phiHao.lt === 'Phụ Mẫu') res += `Tài núp dưới Phụ Mẫu → Vốn đang kẹt ở nhà cửa, hàng hóa, giấy tờ chưa thanh khoản được.`;
                    if (phiHao.lt === 'Huynh Đệ') res += `Tài núp dưới Huynh Đệ → Tiền đang bị bạn bè, đối thủ hoặc người quen chiếm dụng, vay mượn khó đòi.`;
                    if (phiHao.lt === 'Quan Quỷ') res += `Tài núp dưới Quan Quỷ → Tiền bị giam bởi nhà nước, chính quyền, pháp lý hoặc đang bị nợ xấu phong tỏa.`;
                    if (phiHao.lt === 'Tử Tôn') res += `Tài núp dưới Tử Tôn → Tiền đang nằm ở tệp khách hàng, kho hàng hoặc các dự án chưa tung ra.`;
                }
                res += `</li>`;
            }

            if (dt.qTinhChat.includes("Lục Xung")) {
                res += `<li><b>⚡ Chiến lược kinh doanh (Lục Xung):</b> <b>Đánh nhanh thắng nhanh!</b> Không đầu tư ôm vốn dài hạn, cấm hùn vốn lâu năm. Nên làm thương mại, lướt sóng.</li>`;
            } else if (dt.qTinhChat.includes("Lục Hợp")) {
                res += `<li><b>🔗 Chiến lược kinh doanh (Lục Hợp):</b> <b>Tích lũy dài hạn!</b> Làm ăn rất hanh thông, chuỗi cung ứng bền vững, nên kết hợp làm ăn theo quy mô.</li>`;
            }
